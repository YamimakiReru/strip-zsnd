...
import {
  ZsndWavChunk,
  Float32ZeroSoundPredicate,
  ZeroSoundPredicate,
} from "@/services/wav_logic";
...
export default class DetectZsndService {
  ...
  async detect(
    ...
    sampleRate: number,
    minDurationInMs: number = 10,
    threshold: number = -80.0,
  ): Promise<DropoutInfo[]> {
    const totalLengthInSeconds = this._formatNumSamplesInSeconds(
      chunk.size,
      sampleRate,
    );
    console.debug(
      `Length: ${totalLengthInSeconds} / Number of frames: ${chunk.size}`,
    );

    const minDurationInSamples = Math.round(
      (sampleRate * minDurationInMs) / 1000,
    );
  ...
  private _formatNumSamplesInSeconds(
    numSamples: number,
    frameRate: number,
  ): string {
