...
class _PcmIntZeroSoundPredicate(_ZeroSoundPredicateImpl):
    def __init__(self, sample_width_in_bytes: int, threshold_in_db: float):
    ...
    def _are_upper_bytes(
        self, expected: int, frames_as_bytes: bytes, pos_in_bytes: int
    ):
...
class _PcmInt8ZeroSoundPredicate(_ZeroSoundPredicateImpl):
    ...
    @override
    def is_zero_sound_sample(self, frames_as_bytes: bytes, pos_in_bytes: int):
        return self._min_amp <= frames_as_bytes[pos_in_bytes] <= self._max_amp
...
class WavZeroSoundPredicateFactory:
    def create(
        self, wave_reader: ZsndWavReader, threshold_in_db: float
    ) -> ZeroSoundPredicate:
