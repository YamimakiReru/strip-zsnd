...
class StripZsndService(LogMixin):
    ...
    def strip(
        ...
        total_length_in_seconds = self._format_num_samples_in_seconds(
            num_frames, sample_rate
        )
        logger.debug(
            f"Length: {total_length_in_seconds} / Number of frames: {num_frames}"
        )
        ...
        while pos < num_frames:
            ...
            num_prev_trailing_zeros = self._collapse_chunk(
                chunk,
                num_prev_trailing_zeros,
                pos,
                zero_sound_predicate,
                writer,
                sample_rate,
                min_duration_in_samples,
            )
        ...
        if num_prev_trailing_zeros >= min_duration_in_samples:
            self._report_dropout(
                pos - num_prev_trailing_zeros, num_prev_trailing_zeros, sample_rate
            )

    def _collapse_chunk(
        self,
        chunk: ZsndWavChunk,
        num_prev_trailing_zeros: int,
        pos: int,
        zero_sound_predicate: ZeroSoundPredicate,
        writer: ZsndWavWriter | None,
        sample_rate: int,
        ...
        if zero_run_length >= min_duration_in_samples:
            self._report_dropout(
                pos - num_prev_trailing_zeros, zero_run_length, sample_rate
            )

        processed_samples = num_leading_zeros
        for zero_run_start, zero_run_length in chunk.iterate_inner_zero_runs(
            zero_sound_predicate
        ):
            if zero_run_length < min_duration_in_samples:
                continue
    ...
    def _report_dropout(
        self, abs_zero_run_start: int, zero_run_length: int, frame_rate: int
    ):
        logger = self.get_logger()
        s_abs_start = self._format_num_samples_in_seconds(
            abs_zero_run_start, frame_rate
        )
        s_abs_end = self._format_num_samples_in_seconds(
            abs_zero_run_start + zero_run_length, frame_rate
        )
        logger.info(
            _("zsnd.zero_sound_detected")
            % {
                "abs_start": s_abs_start,
                "abs_end": s_abs_end,
                "length": zero_run_length,
            }
        )
